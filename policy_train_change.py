...
import logging
# 这行代码用于配置日志记录的基本设置。具体参数解释如下：
#     level=logging.INFO：设置日志记录的级别为INFO。日志级别从低到高依次为DEBUG、INFO、WARNING、ERROR、CRITICAL。设置为INFO级别意味着只会记录INFO及以上级别的日志信息，即INFO、WARNING、ERROR和CRITICAL级别的日志会被记录，而DEBUG级别的日志不会被记录。
...
def main(args:dict):
    ...
    # wait for free GPU
    # 从环境变量中获取CUDA_VISIBLE_DEVICES的值，如果没有设置则默认为空字符串。
    ...
    waiting_time = 60
    while True:
        # 调用GPUer类的check_all_gpus_idle方法，检查所有GPU是否空闲。该方法返回三个值：free_gpus（空闲的GPU列表）、gpus_num（总GPU数量）、gpu_utilizations（每个GPU的利用率）。
        free_gpus, gpus_num, gpu_utilizations = GPUer.check_all_gpus_idle(gpu_threshold)
        print(f'Free GPUs: {free_gpus}, GPU utilizations: {gpu_utilizations}')
        if len(free_gpus) > 0: # 如果存在空闲的GPU，进入条件判断
            if len(target_gpus) > 0: # 如果指定了目标GPU列表，检查这些GPU是否空闲
                not_free_ids = []
            ...
            time.sleep(waiting_time)
    # 打印最终使用的GPU列表，并将目标GPU列表添加到配置字典中
    print(f"Using GPU: {target_gpus}")
    all_config["target_gpus"] = target_gpus
    # train policy
    set_seed(all_config['seed']) # 设置随机种子，确保结果的可重复性
    best_ckpt_info = train_bc(train_dataloader, val_dataloader, all_config) # 调用train_bc函数进行模型训练，返回最佳检查点信息。
