...
class MujocoEnv(object):
    ...
    def reset(self, fake=False, sleep_time=0):
        ...
        # 如果使用分割图，这里就是分割图 
        # TODO: MMK2还没改
        if self.exec_node.config.use_segmentation_renderer:
            for id in self.exec_node.config.obs_seg_cam_id:
                # obs["images"][f"{id}"] = raw_obs["seg"][id]  #分割图
                obs["images"][f"{id}"] = np.expand_dims(raw_obs["seg"][id], axis=-1)
                # TODO： 确定要给进去三通道的还是单通道的
        else:
            for id in self.exec_node.config.obs_rgb_cam_id:
                obs["images"][f"{id}"] = raw_obs["img"][id][:, :, ::-1]  #原图
